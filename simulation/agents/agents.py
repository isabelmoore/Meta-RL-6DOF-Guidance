...
import numpy as np
from simulation.core.geospatial import dinstance_between_agents, relative_bearing_between_agents
...
class BaseBVRAgent():
    def __init__(self, conf, env):
        ...
        self.UAV_simObj_conf  =  conf.UAV_simObj_conf
        
        # flight dynamics model level configurations
        self.simObj = Aircraft(self.aircraft_simObj_conf)
        self.reset_agent()
        self.reset_ammo()
    ...
    def reset_ammo(self):    
        ...
        if self.agent_parameters.ammo > 0:
            for i in range(self.agent_parameters.ammo):
                self.ammo[str(i)] = UAV(self.UAV_simObj_conf)
