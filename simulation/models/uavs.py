...
import time
from simulation.core.autopilot import UAVPIDAutopilot
from simulation.core.geospatial import dinstance_between_simObj_agent

class UAV(FDMObject):
    def __init__(self, conf, autopilot_cls=None, guidance_cls=None):
        super().__init__(conf)
        self.conf = conf
        
        # Default to PID and PN if not provided
        AutopilotClass = autopilot_cls if autopilot_cls else UAVPIDAutopilot
        GuidanceClass = guidance_cls if guidance_cls else PN
        
        self.UAV_control = AutopilotClass(self)
        self.PN = GuidanceClass(conf)
        
        self.UAV_simulation_config = self.conf.UAV_simulation
        self.UAV_performance = self.conf.UAV_performance
        self.reset_operational_state()

    def get_autopilot_control(self, heading_cmd, alt_cmd):
        """Unified interface for autopilot control."""
        return self.UAV_control.get_control_input(heading_cmd, alt_cmd)
    ...
    def is_target_within_effective_range(self):
        ...
        if self.target_norm <= self.UAV_performance.effective_radius:
    ...
    def is_mach_low(self):
        if self.UAV_control.acceleration_stage_done() and self.get_mach() < self.UAV_performance.target_lost_below_mach:
    ...
    def is_alt_low(self):
        if self.get_altitude() < self.UAV_performance.target_lost_below_alt:
            self.target_lost = True
            self.target_hit = False
            self.active = False
            self.alive = False
    ...
    def is_target_lost(self):
        ...
        if self.target_norm_old > self.target_norm:
            self.lost_count += 1
            if self.lost_count > self.UAV_performance.lost_count:
                self.target_lost = True
                self.target_hit = False
                self.active = False
                self.alive = False
    ...
    def step(self):   
        ...
        for _ in range(self.UAV_simulation_config.Sim_time_step):
    ...
    def _step(self):
        ...
        if self.target_norm > self.conf.UAV_navigation.dive_at:
            altitude_cmd = self.conf.UAV_navigation.alt_cruise
        
        for _ in range(self.UAV_simulation_config.Control_time_step):
                
            aileron_cmd, elevator_cmd, rudder_cmd, throttle_cmd	= self.UAV_control.get_control_input(heading_cmd, altitude_cmd)
            self.command_UAV(aileron_cmd, elevator_cmd, rudder_cmd, throttle_cmd)  
            self.fdm.run()
    ...
    def command_UAV(self, aileron_cmd, elevator_cmd, rudder_cmd, throttle_cmd):
