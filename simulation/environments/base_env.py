# BVR (Beyond Visual Range) base environment.
#
# This is the base Gym environment for BVR air combat, NOT the UAV guidance env.
# The RL agent controls a blue aircraft against a behavior-tree-driven red aircraft.
# Action space: [heading, altitude, throttle] scaled to vehicle limits.
# Observation space: stacked time-series of normalized state features
#     (bearings, headings, mach, altitude, distance, enemy state).
#
# The UAV guidance environment (uav_guidance_env.py) is used for the RL
# guidance-to-intercept task. This BVR env is a separate use case.
...
class BVRBase(gym.Env):
    ...
    def update_state(self):
        """Update the observation state with a rolling window.

        Maintains a fixed-size array of shape obs_shape where each row is one
        timestep's observation. New observations are appended by rolling the
        array left and overwriting the last row, giving the LSTM a short
        ...
        if self.state is None:
            # First call: tile the initial observation to fill the entire window
            self.state = np.tile(obs_nn, (self.obs_shape[0], 1))
        else:
            # Shift window left by one row and insert new observation at the end
            self.state = np.roll(self.state, shift=-1, axis=0)
            self.state[-1,:] = obs_nn

    def update_observation(self):
        """Gather raw observations from both agents into a dict."""
    ...
    def step(self, action):
        ...
        # Scale NN output [-1,1] to physical command ranges
        action = self.from_nn2agent(action, self.blue_agent)

        for i in range(self.conf.step_length):
            # Run both agents for step_length sim seconds per RL step
    ...
    def from_obs2nn(self, agent):
        """Convert raw observations to normalized NN inputs in [-1, 1].

        Encodes angles as sin/cos pairs to avoid discontinuities at 0/360.
        Scales mach, altitude, and distance to [-1, 1] using known bounds.
        """
    ...
    def from_nn2agent(self, action, agent):
        """Convert NN action [-1, 1] to physical commands.

        Channel 0: heading in [psi_min, psi_max] degrees
        Channel 1: altitude in [alt_min, alt_max] meters
        Channel 2: throttle - binary (0.49 normal, 0.69 afterburner)
        """
        # heading
    ...
    def get_reward(self, is_done):
        """Compute reward combining LOS rate shaping, closing velocity, and control penalty.

        Shaping reward (per step):
          - LOS rate: exp(-|omega|^2 / sigma^2) where omega is LOS unit vector change.
            Rewards maintaining a collision course (small LOS rotation = proportional navigation).
          - Closing: positive reward proportional to range decrease per step.
          - Control penalty: penalizes large actions to encourage smooth commands.
        ...
        # --- LOS rate shaping ---
        # Compute LOS unit vector from blue to red in ENU frame
        ...
        # LOS rate via finite difference of unit vector between steps
        if not hasattr(self, '_prev_los_unit_bvr'):
            self._prev_los_unit_bvr = los_unit.copy()
        omega = los_unit - self._prev_los_unit_bvr
        self._prev_los_unit_bvr = los_unit.copy()

        # Gaussian shaping: peak reward (1.0) when omega=0 (perfect collision course)
        sigma = 0.02
