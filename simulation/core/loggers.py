...
class TacviewLogger:
        ...
        for i in self.env.all_agents:
            ...
            for j in i.ammo.keys():
                if i.ammo[j].is_active():
                    self.data_logs[i.conf.agent_name]['ammo'][j].append(self.get_current_UAV_data(i.ammo[j], i))
    ...
    def get_current_UAV_data(self, UAV, agent):
        log = {
        "Time": agent.simObj.get_sim_time_sec(),
        "Longitude": UAV.get_long_gc_deg(),
        "Latitude": UAV.get_lat_gc_deg(),
        "Altitude": UAV.get_altitude(),
        "Roll (deg)": UAV.get_phi(),
        "Pitch (deg)": UAV.get_theta(),
        "Yaw (deg)": UAV.get_psi(),
        }
        return log
